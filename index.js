...
4. 直接输出内容，不要添加额外解释`;
        ...
        // 直接导入角色卡
        async function importCharacterCard(yamlContent, charName) {
            try {
                // 解析 YAML 获取名字和内容
                const nameMatch = yamlContent.match(/chinese_name:\s*(.+)/m);
                const name = nameMatch ? nameMatch[1].trim() : (charName || '新角色');
        ...
        // 直接导入用户人设
        async function importUserPersona(yamlContent, userName) {
            try {
                const nameMatch = yamlContent.match(/chinese_name:\s*(.+)/m);
                const name = nameMatch ? nameMatch[1].trim() : (userName || '新用户');
...
fontSlider.oninput = () => {
    ...
    document.getElementById(PANEL_ID).style.fontSize = `${val}px`;
            ...
            charImport.onclick = async () => {
                ...
                try {
                    await importCharacterCard(charResult.value, '新角色');
                } finally {
            ...
            userImport.onclick = async () => {
                ...
                try {
                    await importUserPersona(userResult.value, '新用户');
                } finally {
