...
// 修复：直接通过ID查找复制按钮，而不是用parentElement
const copyBtn = document.getElementById(`${cardType === 'character' ? 'char' : 'user'}-copy`);
const importBtn = document.getElementById(`${cardType === 'character' ? 'char' : 'user'}-import`);
...
const fullUrl = getFullApiUrl();
const systemPrompt = `根据用户输入，描写外貌、打扮、服饰。注意：只描述客观事实，直接说明是什么样的衣服、什么样的头发、什么样的饰品，避免使用"美丽""漂亮""耀眼""动人"等主观评价性语言。不渲染美感，仅呈现细节。`;
if (btn) { btn.disabled = true; btn.textContent = '生成中...'; }
try {
const res = await fetch(fullUrl, {
method: 'POST',
...
if (!ctx && window.getContext) ctx = window.getContext();
if (!ctx && window.ST && window.ST.getContext) ctx = window.ST.getContext();

if (ctx) {
// 方式1: getWorldInfoList
if (typeof ctx.getWorldInfoList === 'function') {
worldbookList = await ctx.getWorldInfoList();
}
// 方式2: worldInfo.getWorldInfos
else if (ctx.worldInfo && typeof ctx.worldInfo.getWorldInfos === 'function') {
worldbookList = await ctx.worldInfo.getWorldInfos();
}
// 方式3: 直接访问全局 worldInfo
else if (window.worldInfo && typeof window.worldInfo.getWorldInfos === 'function') {
worldbookList = await window.worldInfo.getWorldInfos();
}
// 方式4: 从 localStorage 读取（备用）
else if (localStorage.getItem('world_info')) {
const saved = JSON.parse(localStorage.getItem('world_info') || '{}');
worldbookList = Object.values(saved).map(w => ({ id: w.id || w.uid, name: w.name || w.title }));
}
}
} catch (err) {
...
worldbookList.map(w => `<option value="${w.id}">${w.name}</option>`).join('');
}
}
}
...
if (ctx) {
if (typeof ctx.getWorldInfoContent === 'function') {
return await ctx.getWorldInfoContent(worldbookId);
}
else if (ctx.worldInfo && typeof ctx.worldInfo.getWorldInfoContent === 'function') {
return await ctx.worldInfo.getWorldInfoContent(worldbookId);
}
}
// 备用：从 localStorage 读取
if (localStorage.getItem('world_info')) {
const saved = JSON.parse(localStorage.getItem('world_info') || '{}');
const entry = saved[worldbookId] || Object.values(saved).find(w => w.id === worldbookId || w.uid === worldbookId);
if (entry && entry.entries) {
return entry.entries.map(e => e.content).join('\n\n');
}
}
} catch (err) {
...
<select id="worldbook-select" class="worldbook-select"></select>
</div>
</div>
...
if (id === 'world-extend') {
loadWorldbookList();
const useWorldbook = document.getElementById('use-worldbook');
const container = document.getElementById('worldbook-select-container');
if (useWorldbook && container) {
useWorldbook.onchange = () => {
container.style.display = useWorldbook.checked ? 'block' : 'none';
if (useWorldbook.checked) loadWorldbookList();
};
}
const checkboxes = document.querySelectorAll('.extend-type-checkbox');
