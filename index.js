    ...
    function initPlugin() {
        ...
        async function fetchModels() {
            ...
            if (modelsUrl.endsWith('/chat/completions')) {
                ...
                const baseUrl = modelsUrl.split('/v1')[0];
        ...
        async function generateCharacter(userInput, cardType) {
            ...
            const systemPrompt = cardType === 'character' 
                ? `你是一个专业的角色设定助手。根据用户输入的简单描述，生成一个完整的SillyTavern角色卡。
...
1. 所有字段都要填充，不能留空
2. 生成的内容要符合角色设定
3. first_message 是角色说的第一句话`
                : `你是一个专业的用户设定助手。根据用户输入的简单描述，生成一个完整的用户人设。
请严格按照以下YAML模板格式输出：
user_name: 
nickname: 
age: 
...
要求：所有字段都要填充，不能留空`;
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    toastr.error(`API调用失败: ${response.status} - ${errorText.substring(0, 100)}`);
                    return null;
                }
        ...
        function createPanel() {
            ...
            panel.innerHTML = `
                ...
                <div class="ai-tab-content active" id="tab-api">
                    ...
                    <div class="ai-setting-group" style="font-size: 11px; color: var(--ai-text, #666);">
                        ${config.availableModels && config.availableModels.length > 0 ? `📋 已加载 ${config.availableModels.length} 个模型` : '💡 点击"测试连接"获取模型列表'}
                    </div>
                </div>
