...
if (resultArea) {
resultArea.value = content;
addToHistory('魔法衣橱(多套)', userInput, content);
const copyBtn = resultArea.parentElement?.querySelector('.copy-btn');
if (copyBtn) copyBtn.disabled = false;
...
<h4>套装 ${idx+1}</h4>
<p style="white-space:pre-wrap;">${set.trim()}</p>
</div>
`).join('')}
</div>
`;
...
finally { if (btn) { btn.disabled = false; btn.textContent = '生成多套衣橱'; } }
}// ========== 世界书扩展（新版）==========
