...
ctx.fillText(edge.relation, midX - 15, midY);
});
...
ctx.fillText(node.name.charAt(0), pos.x - 9, pos.y + 7);
ctx.fillStyle = '#666';
ctx.font = '12px sans-serif';
ctx.fillText(node.name.length > 8 ? node.name.substring(0,6)+'..' : node.name, pos.x - 22, pos.y + 38);
});
...
html += `<div>${direction} ${other.name}: ${e.relation}</div>`;
});
} else {
html += '<div>暂无关系</div>';
}
...
const systemPrompt = `分析以下角色之间的关系，输出JSON格式。每条关系包含 from, to, relation（最多6个字）。格式：{"nodes":[{"name":"角色名","desc":"描述"}],"edges":[{"from":0,"to":1,"relation":"盟友"}]}`;
...
if (ctx && ctx.worldInfo && ctx.worldInfo.getWorldInfos) {
worldbookList = await ctx.worldInfo.getWorldInfos();
} else if (ctx && ctx.getWorldInfos) {
worldbookList = await ctx.getWorldInfos();
} else if (ctx && ctx.worldInfos) {
worldbookList = ctx.worldInfos;
} else {
worldbookList = [];
}
}
} catch (err) {
console.warn('获取世界书列表失败:', err);
worldbookList = [];
}
...
worldbookList.map(w => `<option value="${w.id || w.name}">${w.name}</option>`).join('');
}
}
}
...
if (window.SillyTavern && window.SillyTavern.getContext) {
const ctx = window.SillyTavern.getContext();
if (ctx && ctx.worldInfo && ctx.worldInfo.getWorldInfoContent) {
return await ctx.worldInfo.getWorldInfoContent(worldbookId);
} else if (ctx && ctx.getWorldInfoContent) {
return await ctx.getWorldInfoContent(worldbookId);
} else {
const found = worldbookList.find(w => (w.id || w.name) === worldbookId);
if (found && found.content) return found.content;
return `【世界书: ${found?.name || worldbookId}】\n(内容需手动复制)`;
}
}
} catch (err) {
...
if (window.SillyTavern && window.SillyTavern.getContext) {
const ctx = window.SillyTavern.getContext();
if (ctx && ctx.worldInfo && ctx.worldInfo.createWorldInfo) {
const name = prompt('请输入世界书名称', `扩展世界书_${new Date().toLocaleTimeString()}`);
if (!name) return;
await ctx.worldInfo.createWorldInfo({ name, content });
toastr.success(`世界书 "${name}" 已创建`);
return;
} else if (ctx && ctx.createWorldInfo) {
const name = prompt('请输入世界书名称', `扩展世界书_${new Date().toLocaleTimeString()}`);
if (!name) return;
await ctx.createWorldInfo({ name, content });
toastr.success(`世界书 "${name}" 已创建`);
return;
}
}
...
<div style="font-size:11px; color:#666; margin-top:4px;">提示：如果列表为空，请先在酒馆创建世界书</div>
</div>
</div>
...
<div class="button-group">
<button id="world-extend-copy" class="copy-btn" disabled>复制</button>
...
<canvas id="relation-canvas" width="500" height="320" style="width:100%; height:auto; border:1px solid #e0e0e0; border-radius:8px; background:#fff;"></canvas>
</div>
...
charResult.addEventListener('input', () => { const has = !!charResult.value; charCopy.disabled = !has; charImport.disabled = !has; saveDraft('char', charResult.value); if (document.getElementById('tab-world-extend')?.classList.contains('active')) updateSourcePreview(); });
...
worldResult.addEventListener('input', () => { worldCopy.disabled = !worldResult.value; saveDraft('world', worldResult.value); if (document.getElementById('tab-world-extend')?.classList.contains('active')) updateSourcePreview(); });
...
wardrobeClear.onclick = () => { wardrobeResult.value = ''; wardrobeCopy.disabled = true; wardrobeCompare.innerHTML = ''; };
