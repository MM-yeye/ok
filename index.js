// AI Character Generator - 菜单版（可拖动/缩放面板）
(function() {
    setTimeout(() => {
        try {
            initPlugin();
...
2. 生成的内容要符合用户设定`;
        ...
        function createDraggablePanel() {
            ...
            panel.id = PANEL_ID;
            panel.className = 'ai-draggable-panel';
            panel.innerHTML = `
                <div class="ai-panel-header" id="ai-panel-drag-handle">
                    <span>🤖 AI人设生成器</span>
                    <div class="ai-panel-controls">
                        <span class="ai-panel-resize" title="缩放">⤢</span>
            ...
            handle.addEventListener('mousedown', (e) => {
                if (e.target.classList.contains('ai-panel-close') || e.target.classList.contains('ai-panel-resize')) return;
                isDragging = true;
            ...
            const resizeHandle = panel.querySelector('.ai-panel-resize');
            ...
            importBtn.onclick = () => {
                if (generatedContent) {
                    copyToClipboard(generatedContent);
                    toastr.info('已复制到剪贴板，请去角色管理 → 新建角色 → 粘贴');
                }
            };
        }
