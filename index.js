// AI Character Generator - 五页版（含模型列表获取）
(function() {
    setTimeout(() => {
        try {
            initPlugin();
    ...
    function initPlugin() {
        ...
        async function fetchModels() {
            ...
            // 构建模型列表API地址（兼容 OpenAI 格式）
            let modelsUrl = config.apiUrl.replace('/chat/completions', '/models');
            // 如果 apiUrl 是基础地址，尝试不同路径
            if (!modelsUrl.includes('/models')) {
                modelsUrl = config.apiUrl.split('/v1')[0] + '/v1/models';
            }
...
2. 生成的内容要符合用户设定`;
        ...
        function createPanel() {
            ...
            // 构建模型下拉框的选项
            const modelOptions = config.availableModels.length > 0 
                ? config.availableModels.map(m => `<option value="${m}" ${m === config.model ? 'selected' : ''}>${m}</option>`).join('')
                : '<option value="gpt-3.5-turbo">gpt-3.5-turbo</option><option value="gpt-4">gpt-4</option><option value="gpt-4-turbo">gpt-4-turbo</option>';
            ...
            panel.innerHTML = `
                ...
                <div class="ai-tab-content active" id="tab-api">
                    ...
                    <div class="ai-button-group">
                        <button id="ai-test-connection" class="ai-btn ai-btn-secondary">🔌 测试连接并获取模型</button>
                        <button id="ai-save-api" class="ai-btn ai-btn-primary">💾 保存设置</button>
                    </div>
                    <div class="ai-setting-group" id="model-list-info" style="font-size: 11px; color: var(--ai-text, #666);">
                        ${config.availableModels.length > 0 ? `📋 已加载 ${config.availableModels.length} 个模型` : '💡 点击"测试连接"获取模型列表'}
                    </div>
                </div>
                ...
                <div class="ai-tab-content" id="tab-char">
                    <div class="ai-setting-group">
                        <label>简单设定</label>
                        <textarea id="ai-char-input" rows="3" placeholder="例如：前锋，蜡像师的老公，所有屠孝子心里最柔软的地方"></textarea>
                    </div>
                    <div class="ai-button-group">
            ...
            const modelInfo = document.getElementById('model-list-info');
            ...
            document.getElementById('ai-test-connection').onclick = async () => {
                ...
                if (success && config.availableModels.length > 0) {
                    modelInfo.innerHTML = `📋 已加载 ${config.availableModels.length} 个模型，请在下拉框中选择`;
                }
            };
            ...
            fontSlider.oninput = () => {
                ...
                document.querySelectorAll('.ai-setting-group input, .ai-setting-group textarea, .ai-btn, .ai-tab-btn').forEach(el => {
