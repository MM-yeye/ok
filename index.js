...
config.panelLeft = Math.max(5, window.innerWidth - config.panelWidth - 5);
}
}
} catch (err) {}
...
worldbookList = [];
try {
// 方式1: 直接读取 window.world_info (酒馆存储世界书的地方)
if (window.world_info && typeof window.world_info === 'object') {
for (let key in window.world_info) {
if (window.world_info[key] && window.world_info[key].name) {
worldbookList.push({
id: key,
name: window.world_info[key].name
});
}
}
}
// 方式2: 读取 localStorage 里的 world_info
if (worldbookList.length === 0 && localStorage.getItem('world_info')) {
const saved = JSON.parse(localStorage.getItem('world_info') || '{}');
for (let key in saved) {
if (saved[key] && saved[key].name) {
worldbookList.push({
id: key,
name: saved[key].name
});
...
// 方式3: 读取 SillyTavern 扩展存储
if (worldbookList.length === 0 && window.SillyTavern && window.SillyTavern.getContext) {
const ctx = window.SillyTavern.getContext();
if (ctx && ctx.worldInfo && ctx.worldInfo.getWorldInfos) {
worldbookList = ctx.worldInfo.getWorldInfos() || [];
}
}
} catch (err) {
...
try {
// 方式1: 直接从 window.world_info 读取内容
if (window.world_info && window.world_info[worldbookId]) {
const wi = window.world_info[worldbookId];
if (wi.entries && Array.isArray(wi.entries)) {
return wi.entries.map(e => e.content).join('\n\n');
}
if (wi.content) return wi.content;
}
// 方式2: 从 localStorage 读取
if (localStorage.getItem('world_info')) {
const saved = JSON.parse(localStorage.getItem('world_info') || '{}');
const wi = saved[worldbookId];
if (wi && wi.entries) {
return wi.entries.map(e => e.content).join('\n\n');
}
}
// 方式3: 通过 SillyTavern API
if (window.SillyTavern && window.SillyTavern.getContext) {
const ctx = window.SillyTavern.getContext();
if (ctx && ctx.getWorldInfoContent) {
...
<div class="button-group"><button id="char-copy" class="copy-btn">复制</button><button id="char-import" class="import-btn" disabled>导入酒馆</button><button id="char-clear">清空</button></div>
...
<div class="button-group"><button id="user-copy" class="copy-btn">复制</button><button id="user-import" class="import-btn" disabled>导入酒馆</button><button id="user-clear">清空</button></div>
