// AI Character Generator - 完整版（网状关系网）
(function() {
    setTimeout(() => {
        try {
            initPlugin();
  ...
  NSFW_information:
    ...
    limits:`;
        ...
        function removeCharacterFromRelation(index) {
            ...
            if (selectedCharIndex === index) selectedCharIndex = null;
            else if (selectedCharIndex > index) selectedCharIndex--;
            saveConfig();
            refreshCharacterList();
        }
            ...
            container.innerHTML = `
                <div style="display: flex; flex-wrap: wrap; gap: 12px; justify-content: center; padding: 10px;">
                    ${chars.map((c, i) => {
                        ...
                        const relatedTo = relationships[c.name] || [];
                        return `
                            <div class="relation-node" data-index="${i}" style="text-align: center; cursor: pointer; position: relative;">
                                <div style="width: 60px; height: 60px; border-radius: 50%; background: ${isSelected ? '#4A6FA5' : '#e0e0e0'}; color: ${isSelected ? 'white' : '#333'}; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 18px; margin-bottom: 4px; transition: all 0.2s; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                                    ${c.name.charAt(0)}
                                </div>
                                <div style="font-size: 11px; font-weight: ${isSelected ? 'bold' : 'normal'};">${c.name}</div>
                                ${isSelected && relatedTo.length > 0 ? `
                                    <div style="position: absolute; top: 0; left: 70px; white-space: nowrap; font-size: 10px; color: #666;">
                                        ${relatedTo.map(r => `→ ${r.name}`).join(' ')}
                                    </div>
                                ` : ''}
                            </div>
                        `;
                        ...
                        if (detailArea) {
                            if (rels.length > 0) {
                                detailArea.innerHTML = `
                                    <div style="margin-top: 10px; padding: 10px; background: rgba(74,111,165,0.1); border-radius: 8px;">
                                        <strong>${char.name}</strong> 的关系：
                                        ${rels.map(r => `<div style="margin-top: 5px;">→ <strong>${r.name}</strong>: ${r.relation || '未知'}</div>`).join('')}
                                    </div>
                                `;
                            } else {
                                detailArea.innerHTML = `<div style="margin-top: 10px; padding: 10px; color: #888;">暂无关系数据，请先生成关系网</div>`;
                            }
                        }
                    }
                };
...
规则：
1. 分析每对角色之间的关系，用简短词语描述（如：盟友、敌对、暧昧、师徒、家人、同事、主仆、仇敌、暗恋等）
2. 输出格式为JSON，格式如下：
{
  "relationships": {
...
function createPanel() {
    ...
    panel.innerHTML = `
        ...
        <div class="ai-tab-content" id="tab-relation">
            ...
            <div class="ai-setting-group">
                <label>关系网描述</label>
                <textarea id="ai-relation-result" class="editable-result" rows="8" placeholder="生成的关系网会显示在这里..."></textarea>
            </div>
            <div id="ai-relation-detail" style="margin-top: 8px;"></div>
            <div class="ai-button-group">
    ...
    `;
    document.body.appendChild(panel);// 拖动功能
