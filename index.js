// AI Character Generator - 网状关系网版（完整版）
(function() {
    setTimeout(() => {
        try {
            initPlugin();
  ...
  NSFW_information:
    ...
    limits:`;
                ...
                <div style="position: relative; width: 100%; height: ${canvasHeight}px; overflow: visible;">
                    <svg style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none;" viewBox="0 0 ${canvasWidth} ${canvasHeight}">
                        ${lines.map(line => {
                            ...
                            return `
                                <line x1="${from.x}" y1="${from.y}" x2="${to.x}" y2="${to.y}" stroke="${isHighlighted ? '#4A6FA5' : '#aaa'}" stroke-width="${isHighlighted ? '3' : '1.5'}" stroke-dasharray="${isHighlighted ? '0' : '5 3'}" />
        ...
        function updateRelationDetail() {
            ...
            detailArea.innerHTML = `<div style="max-height: 180px; overflow-y: auto;">${details}</div>`;
        }
...
worldResult.value = loadDraft('world');// 关系网逻辑
