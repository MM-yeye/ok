// AI Character Generator - 网状关系网版（修复版）
(function() {
    setTimeout(() => {
        try {
            initPlugin();
  ...
  NSFW_information:
    ...
    limits:`;
        ...
        let selectedCharIndices = [];  // 支持多选

        function addCharacterToRelation(name, desc) {
            config.relationshipCharacters.push({ name, desc, x: 100 + Math.random() * 200, y: 100 + Math.random() * 200 });
            saveConfig();
        ...
        function refreshCharacterCanvas() {
            ...
            container.innerHTML = `
                <div style="position: relative; width: 100%; height: ${canvasHeight}px; overflow: visible;">
                    <svg style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none;" viewBox="0 0 ${canvasWidth} ${canvasHeight}">
                        ${lines.map(line => {
                            ...
                            return `
                                <line x1="${from.x}" y1="${from.y}" x2="${to.x}" y2="${to.y}" stroke="${selectedCharIndices.includes(line.from) || selectedCharIndices.includes(line.to) ? '#4A6FA5' : '#aaa'}" stroke-width="${selectedCharIndices.includes(line.from) || selectedCharIndices.includes(line.to) ? '3' : '1.5'}" stroke-dasharray="${selectedCharIndices.includes(line.from) || selectedCharIndices.includes(line.to) ? '0' : '5 3'}" />
                                <text x="${midX + offsetX}" y="${midY + offsetY}" font-size="10" fill="#666" text-anchor="middle" dominant-baseline="middle" style="pointer-events: none; background: rgba(255,255,255,0.7);">${line.relation.substring(0, 4)}</text>
                            `;
                        }).join('')}
                    </svg>
                    <div style="position: relative; width: 100%; height: 100%;">
                        ${chars.map((c, i) => {
                            ...
                            const rels = relationships[c.name] || [];
                            const shortRels = rels.map(r => r.relation).join(', ');
                            return `
                                <div class="relation-node" data-index="${i}" style="position: absolute; left: ${pos.x - 28}px; top: ${pos.y - 28}px; width: 56px; height: 56px; border-radius: 50%; background: ${isSelected ? '#4A6FA5' : '#e0e0e0'}; color: ${isSelected ? 'white' : '#333'}; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 14px; cursor: pointer; transition: all 0.2s; box-shadow: 0 2px 8px rgba(0,0,0,0.1); z-index: 10;">
                                    ${c.name.charAt(0)}
                                </div>
                                <div style="position: absolute; left: ${pos.x - 40}px; top: ${pos.y + 30}px; width: 80px; text-align: center; font-size: 11px; font-weight: ${isSelected ? 'bold' : 'normal'};">
                                    ${c.name}
                                    ${isSelected && shortRels ? `<div style="font-size: 9px; color: #4A6FA5; margin-top: 2px;">${shortRels}</div>` : ''}
                                </div>
                            `;
                        }).join('')}
                    </div>
        ...
        function updateRelationDetail() {
            ...
            const details = selectedCharIndices.map(idx => {
                ...
                return `
                    <div style="margin-bottom: 12px; padding: 8px; background: rgba(74,111,165,0.1); border-radius: 8px;">
                        <strong>${char.name}</strong>
                        ${rels.length ? rels.map(r => `<div style="margin-top: 4px;">→ ${r.name}: ${r.relation || '关联'}</div>`).join('') : '<div style="margin-top: 4px; color: #888;">暂无关系数据</div>'}
                    </div>
                `;
            }).join('');
            detailArea.innerHTML = `<div style="max-height: 200px; overflow-y: auto;">${details}</div>`;
        }
...
规则：
1. 分析每对角色之间的关系，用简短词语描述（2-4个字，如：盟友、敌对、暧昧、师徒、家人、同事、主仆、仇敌、暗恋、挚友、宿敌等）
2. 输出格式为JSON，格式如下：
{
  "relationships": {
...
function createPanel() {
    ...
    panel.innerHTML = `
        ...
        <div class="ai-tab-content" id="tab-relation">
            ...
            <div class="ai-setting-group">
                ...
                <div id="ai-relation-canvas" style="min-height: 320px; border: 1px solid var(--ai-border, #ddd); border-radius: 8px; overflow: hidden; position: relative; background: var(--ai-bg, #fff);"></div>
            </div>
        ...
        <div class="ai-tab-content" id="tab-size">
            ...
            <div class="ai-setting-group">
                ...
                <input type="range" id="panel-left-slider" min="0" max="500" step="10" value="${config.panelLeft}">
            </div>
            <div class="ai-setting-group">
    ...
    `;
    document.body.appendChild(panel);// 选项卡切换（不加stopPropagation）
