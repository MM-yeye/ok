// AI Character Generator - 主题完美适配版（修复按钮状态）
(function() {
    setTimeout(() => {
        try {
            initPlugin();
  ...
  NSFW_information:
    ...
    limits:`;
        ...
        function refreshHistoryList() {
            ...
            container.querySelectorAll('.history-item').forEach(item => {
                item.onclick = (e) => {
                    ...
                    if (history) {
                        if (history.type === '角色卡') {
                            const resultArea = document.getElementById('ai-char-result');
                            if (resultArea) resultArea.value = history.output;
                        } else if (history.type === '用户人设') {
                            const resultArea = document.getElementById('ai-user-result');
                            if (resultArea) resultArea.value = history.output;
                        } else if (history.type === '世界书') {
                            const resultArea = document.getElementById('ai-world-result');
                            if (resultArea) resultArea.value = history.output;
                        } else if (history.type === '关系网') {
                            const resultArea = document.getElementById('ai-relation-result');
                            if (resultArea) resultArea.value = history.output;
                        }
        ...
        function addRelationChar(name, desc) {
            config.relationChars.push({ name, desc });
            saveConfig();
            refreshRelationCharList();
        }

        function removeRelationChar(index) {
            config.relationChars.splice(index, 1);
            saveConfig();
            refreshRelationCharList();
        }
        ...
        function refreshRelationCharList() {
            ...
            container.innerHTML = config.relationChars.map((c, i) => `
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 6px; border-bottom: 1px solid var(--ai-border, #eee);">
                    ...
                    <button class="ai-btn ai-btn-secondary delete-relation-char" data-index="${i}" style="padding: 2px 6px;">🗑️</button>
                </div>
            `).join('');
        ...
        async function generateRelationText(buttonElement, resultArea) {
            ...
            if (config.relationChars.length < 2) {
            ...
            const systemPrompt = `你是一个专业的关系网分析助手。根据提供的角色列表，分析他们之间的关系，生成详细的关系描述。

规则：
1. 分析每对角色之间的关系，用生动语言描述
2. 输出格式为文本，可以分段，每对关系一段
3. 语言生动有趣，符合角色设定`;
            ...
            try {
                ...
                if (resultArea) {
                    ...
                    addToHistory('关系网', `角色数: ${config.relationChars.length}`, content);
                    // 启用复制按钮
                    const copyBtn = resultArea.parentElement?.querySelector('.ai-copy-btn');
                    if (copyBtn) copyBtn.disabled = false;
                }
        ...
        function addGraphChar(name, desc) {
            config.graphChars.push({ name, desc });
            saveConfig();
        ...
        function removeGraphChar(index) {
            config.graphChars.splice(index, 1);
        ...
        function refreshGraphCharList() {
            ...
            if (!container) return;
            if (config.graphChars.length === 0) {
                container.innerHTML = '<div style="font-size: 11px; color: #888; text-align: center; padding: 20px;">暂无角色，点击上方添加</div>';
                return;
            }
            container.innerHTML = config.graphChars.map((c, i) => `
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 6px; border-bottom: 1px solid var(--ai-border, #eee);">
                    <span><strong>${c.name}</strong><span style="font-size: 10px; color: #888; margin-left: 8px;">${c.desc ? c.desc.substring(0, 30) : ''}</span></span>
                    <button class="ai-btn ai-btn-secondary delete-graph-char" data-index="${i}" style="padding: 2px 6px;">🗑️</button>
                </div>
            `).join('');
                ...
                node.onclick = (e) => {
                    ...
                    if (e.ctrlKey || e.metaKey) {
                        ...
                        if (selectedCharIndices.length === 1 && selectedCharIndices[0] === idx) {
                            const char = config.graphChars[idx];
        ...
        async function generateGraphData(buttonElement) {
            ...
            const charList = config.graphChars.map(c => `- ${c.name}: ${c.desc || '暂无描述'}`).join('\n');
...
3. 确保每对关系双向都有记录`;
            ...
            try {
                ...
                try {
                    ...
                    saveConfig();
                    addToHistory('关系图谱', `角色数: ${config.graphChars.length}`, parsed.details || content);
                    refreshGraphCanvas();
...
function applyTheme() {
    ...
    if (config.themeMode === 'fixed') {
        ...
        const styleContent = `
            ...
            #${PANEL_ID} .ai-tab-btn.active {
                border-bottom: 2px solid #000000 !important;
            }
            #${PANEL_ID} .ai-panel-header {
        ...
        if (currentStyleElement) currentStyleElement.remove();
        currentStyleElement = document.createElement('style');
        currentStyleElement.id = 'ai-fixed-theme-style';
        currentStyleElement.textContent = styleContent;
        ...
        panelEl.style.cssText = `
            ...
            color: var(--SmartThemeBodyColor, #333) !important;
            border: 1px solid var(--SmartThemeBorderColor, #ddd) !important;
...
function createPanel() {
    ...
    panel.innerHTML = `
        ...
        <div class="ai-tab-content" id="tab-char">
            ...
            <div class="ai-button-group">
                ...
                <button id="ai-char-import" class="ai-btn ai-btn-primary ai-import-btn" disabled>📥 直接导入角色卡</button>
        ...
        <div class="ai-tab-content" id="tab-user">
            ...
            <div class="ai-button-group">
                ...
                <button id="ai-user-import" class="ai-btn ai-btn-primary ai-import-btn" disabled>📥 直接导入用户人设</button>
        ...
        <div class="ai-tab-content" id="tab-relation-text">
            <div class="ai-setting-group">
                <label>👥 角色列表</label>
                <div id="ai-relation-char-list" style="max-height: 150px; overflow-y: auto; border: 1px solid var(--ai-border, #ddd); border-radius: 8px; margin-bottom: 8px;"></div>
        ...
        <div class="ai-tab-content" id="tab-relation-graph">
            <div class="ai-setting-group">
                <label>👥 角色列表</label>
                <div id="ai-graph-char-list" style="max-height: 150px; overflow-y: auto; border: 1px solid var(--ai-border, #ddd); border-radius: 8px; margin-bottom: 8px;"></div>
                <div style="display: flex; gap: 6px; margin-bottom: 8px;">
                    <input type="text" id="ai-graph-char-name" placeholder="角色名称" style="flex: 1;">
                    <button id="ai-graph-add-char" class="ai-btn ai-btn-secondary">➕ 添加</button>
                </div>
                <div style="margin-bottom: 8px;">
            ...
            <div class="ai-setting-group">
                ...
                <div id="ai-graph-canvas" style="min-height: 320px; border: 1px solid var(--ai-border, #ddd); border-radius: 8px; background: var(--ai-bg, #fff);"></div>
            </div>
            ...
            <div class="ai-button-group">
                <button id="ai-graph-copy" class="ai-btn ai-btn-secondary ai-copy-btn" disabled>📋 复制详情</button>
                <button id="ai-graph-clear" class="ai-btn ai-btn-secondary">🗑️ 清空</button>
            </div>
            <div class="ai-setting-group">
                <label>💾 保存为模板</label>
                <div style="display: flex; gap: 6px;">
                    <input type="text" id="ai-graph-template-name" placeholder="模板名称" style="flex: 2;">
                    <button id="ai-graph-save-template" class="ai-btn ai-btn-secondary">保存</button>
                </div>
            </div>
        </div>
    ...
    document.body.appendChild(panel);// 选项卡切换
                ...
                btn.onclick = () => {
                    ...
                    if (resultArea && resultArea.classList.contains('editable-result')) {
                        ...
                        if (draft && !resultArea.value) {
                            ...
                            const importBtn = resultArea.parentElement?.querySelector('.ai-import-btn');
                            if (copyBtn) copyBtn.disabled = false;
                            if (importBtn && (tabId === 'char' || tabId === 'user')) importBtn.disabled = false;
                        }
                    }
            ...
            relationClear.onclick = () => { 
                relationResult.value = ''; 
                relationCopy.disabled = true; 
                saveDraft('relation-text', '');
            };
        ...
        style.textContent = `
            ...
            .graph-node:hover, .relation-node:hover {
