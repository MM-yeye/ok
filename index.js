...
if (window.SillyTavern && window.SillyTavern.getContext) {
const ctx = window.SillyTavern.getContext();
if (ctx && ctx.getWorldInfoList) {
worldbookList = await ctx.getWorldInfoList();
} else if (ctx && ctx.worldInfo && ctx.worldInfo.getWorldInfos) {
worldbookList = await ctx.worldInfo.getWorldInfos();
} else if (window.worldInfo && window.worldInfo.getWorldInfos) {
...
} else if (ctx && ctx.worldInfo && ctx.worldInfo.getWorldInfoContent) {
return await ctx.worldInfo.getWorldInfoContent(worldbookId);
} else if (window.worldInfo && window.worldInfo.getWorldInfoContent) {
return await window.worldInfo.getWorldInfoContent(worldbookId);
}
}
} catch (err) {
...
if (!sourceContent) { toastr.warning('请选择已有世界书或先生成内容'); return; }
} else {
